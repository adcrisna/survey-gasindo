...
@section('content')
  <section class="content-header">
    <ol class="breadcrumb">
      ...
      <li class="active">Data CSI Layanan</li>
    </ol>
    <br/>
  </section>
  <section class="content">
    ...
    <div class="box-header">
        <h3 class="box-title">Data CSI Layanan </h3>
            <div class="box-tools pull-right">
                <a href="{{ route('kelola_csi_layanan') }}" class="button btn btn-xs btn-primary" onclick="return confirm('apakah anda yakin ?')"><i class="fa fa-edit"></i> Kelola Data CSI Layanan</a> &nbsp;
                <!-- <a href="{{ route('hapus_csi_layanan') }}" class="button btn btn-xs btn-danger" onclick="return confirm('apakah anda yakin ?')"><i class="fa fa-trash"></i> Hapus Semua Data CSI Layanan</a> -->
            </div>
        </div>
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>{{ round($value->mss_layanan,2) }}</td>
                            <td style="display:none;">{{ round($total1 += $value->mis_layanan,2) }}</td>
                            <td style="display:none;">{{ round($total2 += $value->mss_layanan,2) }}</td>
                        </tr>
                        @endforeach
                    ...
                    <thead>
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td style="display:none;">{{ round($total3 += $value->mis_layanan,2) }}</td>
                            <td style="display:none;">{{ round($total4 += $value->wf_layanan,2) }}</td>
                        </tr>
                        @endforeach
                        <tr style="background-color: yellow;">
                            <th>Total</th>
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>{{ round($value->ws_layanan,2) }}</td>
                            <td style="display:none;">{{ round($total5 += $value->mss_layanan,2) }}</td>
                            <td style="display:none;">{{ round($total6 += $value->wf_layanan,2) }}</td>
                            <td style="display:none;">{{ round($total7 += $value->ws_layanan,2) }}</td>
                        </tr>
                        @endforeach
                        <tr style="background-color: yellow;">
                            <th>Total</th>
